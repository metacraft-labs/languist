...
proc `$`*(n: PType): string
proc e*(n: PNode): string

proc text*(n: TNode, depth: int): string

proc text*(n: PNode, depth: int): string =
  if n == nil:
...
proc text*(n: TNode, depth: int): string =
  ...
  of nkCharLit..nkUInt64Lit:
    return "$1$2($3)" % [indent, $n.kind, $n.intVal]
  of nkFloatLit..nkFloat128Lit:
    return "$1$2($3)" % [indent, $n.kind, $n.floatVal]
  of nkStrLit..nkTripleStrLit:
    return "$1$2($3)" % [indent, $n.kind, n.strVal]
  of nkSym:
    sons = $n.sym[]
  of nkIdent:
    return "$1Ident(\"$2\")" % [indent, $n.ident[].s]
  else:
    sons = "\n$1\n$2" % [
  ...
  result = "$1$2:$3" % [indent, $n.kind, sons] #, typ: $4, info: $5)" % [indent, $n.kind, sons, $n.typ, $n.info]
...
proc `$`*(n: TSym): string =
  ...
  if n.kind == skModule:
    ...
    m.add(", typ: $1, ast: $2" % [n.typ.typeToString(), text(n.ast, 0)])
  else:
    m.add(", typ: $1" % n.typ.typeToString())

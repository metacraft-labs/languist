# import
  # strformat, os, strutils, sequtils, tables, ast_dsl
...
proc newCompiler*(db: TraceDb, command: string): Compiler =
...
proc generalize(typ: Type): Type =
  assert typ.kind == T.Method

  result = Type(kind: T.Method, args: typ.args.mapIt(simpleType("auto")), returnType: simpleType("auto"))
